...
# Defining DeepLabV3+ model
model = smp.DeepLabV3Plus(
    encoder_name="resnet18",
    encoder_weights=None,           #No pretraining
    in_channels=1,                  #Grayscale images
    classes=3                       #Background, Edge Artifact, Defect
).to(device)
...
save_path = "/Weights_Deeplabv3+.pth"
...
for epoch in range(num_epochs):
    ...
    print(f"Train Loss: {avg_train_loss:.6f} | Val Loss: {avg_val_loss:.6f} | ⏱Time: {duration:.2f}s")
...
ax.set_ylim(0, 1.05)
ax.legend(loc='lower left', bbox_to_anchor=(0, +0.01), ncol=1)
